...
void Player::update() {
    if(moveLeft && speedup &&rect.x > 0 ) rect.x -= speed+10;
    else if (moveLeft && rect.x > 0) rect.x -= speed;
    if(moveRight && speedup &&rect.x < 800 - rect.w) rect.x += speed+10;
    else if (moveRight && rect.x < 800 - rect.w) rect.x += speed;
